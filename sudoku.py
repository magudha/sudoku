def find_next_empty(puzzle):
    ...
    # we ae using 0-8 for our indices
    for r in range(9):
...
def is_valid(puzzle, guess, row, col):
    ...
    # and then the square
    # this is tricky, but we want 1 // 3 = 0, 5 // 3 = 1, ...
    col_start = (col // 3) * 3 # this is tricky, but we want to get where the 3x3 square start
...
def solve_sudoku(puzzle):
    ...
    # step 2: if there's a place to put a number thenmake a guess between 1 and 9
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3, ...9
        # step 3: check if this is valid guess
        if is_valid(puzzle, guess, row, col):
            # step 3.1: if this is valid, thn pace that guess on the puzzle!
